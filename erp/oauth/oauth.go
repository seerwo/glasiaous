...
func (oauth *Oauth) GetUserAccessToken(code string) (result ResAccessToken, err error) {
	...
	if result.Count != 0 {
		err = fmt.Errorf("GetUserAccessToken error : errcode=%v , errmsg=%v", result.Count, result.Messages)
		return
	}
	return
}
...
type UserInfo struct {
	...
	DeptRefer string `json:"deptRefer"`
	DeptUpdate string `json:"deptUpdate"`
	RightApproval int `json:"rightApproval"`
	RightApprovalAr int `json:"rightApproval_Ar"`
	RightApprovalAp int `json:"rightApproval_Ap"`
	RightApprovalSal int `json:"rightApproval_Sal"`
	RightApprovalPur int `json:"rightApproval_Pur"`
	RightApprovalPo int `json:"rightApproval_Po"`
	RightApprovalPap int `json:"rightApproval_Pap"`
	DeptApproval string `json:"deptApproval"`
	TranslateFlag int `json:"translateFlag"`
	StandardCurrency string `json:"standardCurrency"`
	LicenceSingFlag int `json:"licenceSingFlag"`
	LicenceChkFlag int `json:"licenceChkFlag"`
	PaySubstitute int `json:"paySubstitute"`
	Offset float64 `json:"offset"`
	Roles string `json:"roles"`
	IsMultiBiz int `json:"isMultiBiz"`
	DecimalPoint int `json:"decimalPoint"`
	DigitSeparator int `json:"digitSeparator"`
	CommonPatternEdit int `json:"commonPatternEdit"`
	GridItemSettingEdit int `json:"gridItem_Setting_Edit"`
	Theme string `json:"theme"`
	RightUpdate int `json:"rightUpdate"`
	RightUpdateMaster int `json:"rightUpdate_Master"`
	DeptApprovalBudget string `json:"dept_Approval_Budget"`
	titleLagControl int `json:"titleLagControl"`
	CompanyCode string `json:"companyCode"`
	UserCode string `json:"userCode"`
	Language string `json:"language"`
	IsGroupUser int `json:"isGroupUser"`
	//util.CommonError
...
func (oauth *Oauth) GetUserInfo(accessToken, openID, lang string) (result UserInfo, err error) {
	...
	if result.Count != 0 {
		err = fmt.Errorf("GetUserInfo error : errcode=%v , errmsg=%v", result.Count, result.Messages)
		return
	}
	return
}
